...
class ImitationVecTask():
    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        ...
        self.num_point_cloud_feature_dim = task.num_point_cloud_feature_dim
...
# C++ CPU Class
class ImitationVecTaskCPU(ImitationVecTask):
...
# C++ GPU Class
class ImitationVecTaskGPU(ImitationVecTask):
    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        super().__init__(task, rl_device, clip_observations=clip_observations, clip_actions=clip_actions)
...
class ImitationVecTaskPython(ImitationVecTask):
    ...
    def step_collect_demonstration(self, actions):
        actions_tensor = torch.clamp(actions, -self.clip_actions, self.clip_actions)

        self.task.step(actions_tensor)

        return torch.clamp(self.task.obs_buf[:, :self.num_obs - 3 * self.num_point_cloud_feature_dim], -self.clip_obs, self.clip_obs).to(self.rl_device), self.task.rew_buf.to(self.rl_device), self.task.reset_buf.to(self.rl_device), self.task.extras, torch.clamp(self.task.student_obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device)

    def reset_collect_demonstration(self):
        actions = 0.01 * (1 - 2 * torch.rand([self.task.num_envs, self.task.num_actions], dtype=torch.float32, device=self.rl_device))

        # step the simulator
        self.task.step(actions)

        return torch.clamp(self.task.obs_buf[:, :self.num_obs - 3 * self.num_point_cloud_feature_dim], -self.clip_obs, self.clip_obs).to(self.rl_device), torch.clamp(self.task.student_obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device)
