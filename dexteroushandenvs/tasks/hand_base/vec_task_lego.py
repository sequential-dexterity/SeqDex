...
class VecTask():
    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        ...
        self.retri_info = {}
        self.retri_info['action_space'] = self.act_space
        self.retri_info['observation_space'] = spaces.Box(np.ones(task.searching_num_obs) * -np.Inf, np.ones(task.searching_num_obs) * np.Inf)
        self.retri_info['state_space'] = spaces.Box(np.ones(task.searching_num_states) * -np.Inf, np.ones(task.searching_num_states) * np.Inf)
        self.retri_info['agents'] = 1

        self.grasp_info = {}
        self.grasp_info['action_space'] = self.act_space
        self.grasp_info['observation_space'] = spaces.Box(np.ones(task.grasping_num_obs) * -np.Inf, np.ones(task.grasping_num_obs) * np.Inf)
        self.grasp_info['state_space'] = spaces.Box(np.ones(task.grasping_num_states) * -np.Inf, np.ones(task.grasping_num_states) * np.Inf)
        self.grasp_info['agents'] = 1

        self.insert_info = {}
        self.insert_info['action_space'] = self.act_space
        self.insert_info['observation_space'] = spaces.Box(np.ones(task.insertion_num_obs) * -np.Inf, np.ones(task.insertion_num_obs) * np.Inf)
        self.insert_info['state_space'] = spaces.Box(np.ones(task.insertion_num_states) * -np.Inf, np.ones(task.insertion_num_states) * np.Inf)
        self.insert_info['agents'] = 1

    def get_retri_env_info(self):
        return self.retri_info

    def get_grasp_env_info(self):
        return self.grasp_info

    def get_insert_env_info(self):
        return self.insert_info
...
class LegoVecTaskPython(VecTask):
    ...
    def step(self, actions):
        ...
        self.task.extras["before_obs"] = torch.clamp(self.task.extras["before_obs"], -self.clip_obs, self.clip_obs).to(self.rl_device)
        self.task.extras["before_states"] = torch.clamp(self.task.extras["before_states"], -self.clip_obs, self.clip_obs).to(self.rl_device)
        self.task.extras["after_obs"] = torch.clamp(self.task.extras["after_obs"], -self.clip_obs, self.clip_obs).to(self.rl_device)
        self.task.extras["after_states"] = torch.clamp(self.task.extras["after_states"], -self.clip_obs, self.clip_obs).to(self.rl_device)
    ...
    def reset(self):
        ...
        # Clamp main obs buf and add it to obs dict
        obs_dict["obs"] = torch.clamp(self.task.extras["after_obs"], -self.clip_obs, self.clip_obs).to(self.rl_device)
        obs_dict["states"] = torch.clamp(self.task.extras["after_states"], -self.clip_obs, self.clip_obs).to(self.rl_device)
        obs_dict["before_obs"] = torch.clamp(self.task.extras["before_obs"], -self.clip_obs, self.clip_obs).to(self.rl_device)
        obs_dict["before_states"] = torch.clamp(self.task.extras["before_states"], -self.clip_obs, self.clip_obs).to(self.rl_device)
